...
void Renderer::Render(GameObject * gameObject)
{
	...
	vertexShader->SetMatrix4x4("world", gameObject->GetWorldMatrix()); //Set vertex shader's world matrix to this object's world matrix
...
void Renderer::RenderInstanced(vector<GameObject *> instancedGameObjects)
{
	...
	//Load shaders
	SimpleVertexShader* instancingVS = AssetManager::getInstance().GetVShader("InstancingVShader");
	SimplePixelShader* basePS = AssetManager::getInstance().GetPShader("BasePixelShader");
